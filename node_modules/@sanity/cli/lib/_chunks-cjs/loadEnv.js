...
function isSanityV2StudioRoot(basePath) {
  try {
    ...
    return isRoot && getCliConfig.debug("Found Sanity v2 studio root at %s", basePath), isRoot;
  } catch {
    return !1;
  }
}
...
`), value = value.replace(/\\r/g, "\r")), obj[key] = value;
  ...
  function _configVault(options) {
    const debug = !!(options && options.debug), quiet = options && "quiet" in options ? options.quiet : !0;
  ...
  function populate(processEnv, parsed, options = {}) {
    const debug = !!(options && options.debug), override = !!(options && options.override);
