...
const DocumentReporter = (props) => {
  ...
  const {
    ...
    dispatchDragEndEvent
  } = useDragEndEvents(), sendTelemetry = PointerEvents.useTelemetry(), overlayEventHandler = react.useCallback((message) => {
    ...
    dispatch(message);
  }, [comlink2, dispatch, dispatchDragEndEvent, onDrag, sendTelemetry]), controller = useController(rootElement, overlayEventHandler, inFrame, inPopUp);
  ...
  const closeContextMenu = t10, optimisticActorReady = mutations.useOptimisticActorReady(), componentResolver = optimisticActorReady ? _componentResolver : void 0;
  let t11;
  bb0: {
    ...
    let t122;
    $[15] !== comlink2 || $[16] !== componentResolver || $[17] !== dragMinimapTransition || $[18] !== dragShowMinimap || $[19] !== elements || $[20] !== inFrame || $[21] !== isDragging || $[22] !== optimisticActorReady || $[23] !== props.plugins || $[24] !== wasMaybeCollapsed ? (t122 = elements.filter(_temp0).map((t132) => {
      const {
        id,
        element: element_0,
        focused,
    ...
    }), $[15] = comlink2, $[16] = componentResolver, $[17] = dragMinimapTransition, $[18] = dragShowMinimap, $[19] = elements, $[20] = inFrame, $[21] = isDragging, $[22] = optimisticActorReady, $[23] = props.plugins, $[24] = wasMaybeCollapsed, $[25] = t122) : t122 = $[25], t11 = t122;
  }
